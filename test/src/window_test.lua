...
-- `meta.requires: windows.admin` and the base's compositor asks the logged-on
...
local function define_tests()
    test.describe("Task Manager window entry", function()
        ...
        test.it("runs under its own policy, which reads the runtime and may end a process", function()
            local window: any = assert(registry.get("windows.taskman:window"))
            local policies = (window.data and window.data.security and window.data.security.policies) or {}
            test.eq(table.concat(policies, ","), "windows.taskman:window_scope")
            local scope: any = assert(registry.get("windows.taskman:window_scope"))
